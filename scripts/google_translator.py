...
class GeminiTranslator:
    ...
    def _translate_title(self, title: str) -> str:
        ...
        prompt = f'당신은 음악 평론 전문 번역가입니다. 다음 음악 칼럼 제목을 한국어로 번역해주세요. 아티스트명은 원문도 함께 표기 (ex: 테일러 스위프트(Taylor Swift)). 괄호 안 영문은 반드시 대문자로 시작할 것 (ex: (Taylor Swift) O, (taylor swift) X). 부연 설명 없이 번역된 제목만 출력. 자연스러운 한국어로 번역. 제목: {title}'
...
3. 영문 병기는 기본적으로 하지 않는다. 아래 조건을 모두 충족할 때만 예외적으로 허용:
   — 한국어 표기만으로는 의미를 전혀 추론할 수 없는 극히 전문적인 용어일 것
   — 힙합·팝·록·재즈·R&B·소울·인디·코러스·비트·훅·루프·보컬 등 이미 한국어로 정착된 단어는 병기 금지
4. 병기가 꼭 필요한 경우 형식은 두 가지 중 하나:
   a) 영어 표기만으로 충분한 경우: 한글(English) — ex: 폴리리듬(Polyrhythm)
   b) 추가 설명이 필요한 경우: 한글(English; 한글 설명) — ex: 폴리리듬(Polyrhythm; 둘 이상의 리듬이 동시에 진행되는 기법)
   — 괄호 안 영문은 반드시 대문자로 시작할 것 (ex: (Polyrhythm) O, (polyrhythm) X)
   — 연도·숫자·소문자로 시작하는 내용은 절대 괄호 병기하지 말 것 (ex: (2024), (feat. ...) 형태 금지)
5. 자연스러운 한국어로 번역
...
{content}'''
        ...
        # 규칙 4 — 프론트엔드 annotation 렌더러가 대문자 시작 괄호만 인식하므로 소문자·숫자 시작은 병기 금지
